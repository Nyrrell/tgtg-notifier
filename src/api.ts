...
const loginByEmail = async (email: string) => TGTG.post(endpoint['authByEmail'], {
    "device_type": "ANDROID",
    "email": email,
});

const authPolling = (email: string, pollingId: string) => TGTG.post(endpoint['authPolling'], {
    "device_type": "ANDROID",
    "email": email,
    "request_polling_id": pollingId,
});

const refreshToken = (accessToken: string, refreshToken: string) => TGTG.post(endpoint['refreshToken'], {
...
const getItems = (accessToken: string, userId: string, withStock: boolean = true, favorite: boolean = true) => TGTG.post(endpoint['item'], {
